...
总有一些时候、SpringBoot提供的Starter功能不能满足使用,自定义Starter;

1. 自定义starter中需要包含一个autoconfigurate包、用来存放自动配置类；

   starter  + autoconfigurate  =>  xxx-spring-boot-starter

   - springBoot提供的starter 一般命名为  spring-boot-starter-xxxx;
   - 相反的则是代表自定义starter、例如SpringBoot是没有提供mybatis的starter、 因此mybatis官方定义提供了mybatis-spring-boot-starter;

2. autoconfigure包中、需要创建/META-INF/spring.factories、 并使用EnableAutoConfiguration的值、使springboot启动去加载这个配置类；

   - 提供一个@EnablexxxAutoConfiguration注解、使用@Import导入我们的自动配置类；

3. 编写自动配置类 xxxAutoConfiguration  ==> 绑定xxxProperties类 ==> 绑定yaml配置

   - 可能会使用到@ConditionOnBean、 @ConditionOnClass、@Bean、@Configuration等注解；

4. 项目引入starter => 记载xxxAutoConfiguraion = 容器中放入组件 ---- `绑定xxxProperties` ---- 配置项
